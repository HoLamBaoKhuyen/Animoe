...
const Episodes = (props: any) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const result = await axios(`${BASE_API}/anime/${id}/episodes`);
  ...
  return data ? (
    <Box>
      <Grid container rowSpacing={2}>
        ...
        {dataPagi.currentData().map((ep: any, index: number) => (
          <Box key={index} width="100%" my={1}>
            ...
            >
              ...
              <Grid item xs={3} textAlign="center">
                <Typography
                  variant="subtitle1"
                  sx={{ color: theme.color._100, fontWeight: 500 }}
                >
                  {format_date(ep.aired)}
                </Typography>
              </Grid>
              <Grid item xs={1}>
