...
const SearchBar = () => {
  ...
  const handleKeyPress = (event: any) => {
    if (event.key === "Enter") {
      return navigate(`/${mode.toLowerCase()}-search?q=${wordEntered}`);
    }
  };
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetch(
        `https://api.jikan.moe/v4/${mode.toLowerCase()}?q=${wordEntered}&limit=5`
      )
        ...
        .then((json) => {
          const newFilter: any[] = json.data.filter((item: any) => {
            return item.title
              .toLowerCase()
              .includes(wordEntered.toLowerCase().trim());
          });
          if (wordEntered === "") {
            setFilteredData([]);
          } else {
